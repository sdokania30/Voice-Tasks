...
import { Priority, Task, ReminderOption, Trade, Tag, RecurringPayment } from '../types';
...
const processedTasksSchema = {
    ...
    items: {
        ...
        properties: {
            ...
            rawText: { type: Type.STRING, description: "The original text segment from the transcript that corresponds to this single task." },
            when: { type: Type.STRING, description: "The specific due date, time, and estimated completion time (ETA) mentioned. Consolidate all into this one field. Format it clearly, e.g., 'Nov 6, 2024 at 6:00 PM (ETA: 2h)'. Avoid relative terms like 'today' or 'tomorrow'." },
            priority: {
                type: Type.STRING,
                enum: Object.values(Priority),
                description: "The priority of the task. Infer from keywords like 'urgent', 'asap', or importance. Default to Medium if unsure."
            },
            tag: {
                type: Type.STRING,
            ...
            reminder: {
                ...
                description: "When to send a reminder. Infer from phrases like 'remind me 15 minutes before'. If no reminder is specified, this field must be null."
            },
            subtasks: {
                type: Type.ARRAY,
                items: {
                ...
                description: "Break down complex tasks into exactly 1 or 2 smaller subtasks. Do not exceed 2 subtasks."
            }
        },
...
export const processTranscriptToTasks = async (transcript: string): Promise<Partial<Task>[]> => {
    if (!transcript.trim()) {
        return [];
    }
    ...
    try {
        ...
        return tasks.map((task: any) => ({
            ...
            reminder: task.reminder === 'null' || task.reminder === ReminderOption.NONE ? undefined : task.reminder,
            tag: task.tag === 'null' || !Object.values(Tag).includes(task.tag) ? Tag.OTHERS : task.tag as Tag,
        }));
    } catch (error) {
...
const processedTradesSchema = {
    ...
    items: {
        type: Type.OBJECT,
        properties: {
            stockName: { type: Type.STRING, description: "The name or ticker symbol of the stock." },
            entryPrice: { type: Type.NUMBER, description: "The price at which the stock was bought." },
            quantity: { type: Type.INTEGER, description: "The number of shares traded." },
            stopLoss: { 
                type: Type.NUMBER, 
                description: "The calculated stop-loss as a percentage. This is a crucial field. If the user provides a price, calculate the percentage relative to the entry price."
            },
            entryDate: { 
                type: Type.STRING,
                description: "The date of the trade, converted into a machine-readable ISO 8601 date string (YYYY-MM-DD)."
            },
            reason: {
                type: Type.STRING,
                description: "A concise, well-written reason for taking the trade, based on the user's explanation. If no reason is provided, this should be an empty string."
            }
        },
        required: ['stockName', 'entryPrice', 'quantity', 'stopLoss', 'entryDate', 'reason']
    }
};

export const processTranscriptToTrades = async (transcript: string): Promise<Partial<Trade>[]> => {
    if (!transcript.trim()) {
        return [];
    }
    const currentDate = new Date().toISOString();
...
const processedPaymentsSchema = {
    ...
    items: {
        type: Type.OBJECT,
        properties: {
            serviceName: { type: Type.STRING, description: "The name of the service or bill (e.g., Netflix, Rent, Gym)." },
            amount: { type: Type.NUMBER, description: "The payment amount. Assume currency is Rupees (INR) unless specified otherwise." },
            dueDay: { type: Type.INTEGER, description: "The day of the month the payment is due (1-31)." }
        },
        required: ['serviceName', 'amount', 'dueDay']
    }
};

export const processTranscriptToPayments = async (transcript: string): Promise<Partial<RecurringPayment>[]> => {
    if (!transcript.trim()) {
        return [];
    }
    try {
